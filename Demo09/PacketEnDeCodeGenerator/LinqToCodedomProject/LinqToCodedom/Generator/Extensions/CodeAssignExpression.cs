...
{
    public class CodeAssignExpression : CodeSnippetExpression, ICustomCodeDomObject
    {
        private CodeParameterDeclarationExpression m_typeReference;
        private CodeExpression m_expression;

        public CodeAssignExpression(CodeParameterDeclarationExpression type, CodeExpression expression)
        {
            m_typeReference = type;
            m_expression = expression;
        }

        public CodeParameterDeclarationExpression TypeReference
        {
            ...
            switch (language)
            {
                case CodeDomGenerator.Language.CSharp:
                    using (Microsoft.CSharp.CSharpCodeProvider provider = new Microsoft.CSharp.CSharpCodeProvider())
                    {
                        ...
                        {
                            provider.GenerateCodeFromExpression(TypeReference, tw, opts);
                            tw.Write(" = ");
