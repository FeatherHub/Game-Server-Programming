            ...
            while (IsNetworkThreadRunning)
            {
                ...
                if (recvData.Count > 0)
                {
                    ...
                    while (true)
                    {
                        ...
                        packet.PacketID = BitConverter.ToInt16(data.Array, data.Offset);
                        packet.DataSize = BitConverter.ToInt16(data.Array, data.Offset+2);
                        packet.BodyData = new byte[packet.DataSize];
                        Buffer.BlockCopy(recvData.Array, (recvData.Offset + 12), packet.BodyData, 0, (data.Count - 4));
